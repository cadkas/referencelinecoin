...
#if defined(NDEBUG)
# error "Referencelinecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Referencelinecoin Signed Message:\n";
...
{
    // Referencelinecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Referencelinecoin: 840k blocks in ~4 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Referencelinecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Referencelinecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "02/Mar/2018 Facebook starts News Feed in Germany";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfe, 0xce, 0xb7, 0xdf }; // Referencelinecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ReferencelinecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ReferencelinecoinMiner : ProcessBlock, block not accepted");
    }
...
void static ReferencelinecoinMiner(CWallet *pwallet)
{
    ...
    RenameThread("referencelinecoin-miner");
    ...
    try { loop {
        ...
        printf("Running ReferencelinecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ReferencelinecoinMiner terminated\n");
        throw;
    }
}
